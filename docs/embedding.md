...
The production-grade version of this template is the conductor's own backend
client, [`conductor/src/proxy/client.ts`](../conductor/src/proxy/client.ts): it
adds request timeouts, EPIPE-safe writes, reconnect loops, and a
SIGTERM→SIGKILL close escalation on top of the same skeleton.
...
- If you expose the server to a model loop, add per-request timeouts so a hung
  backend cannot wedge your agent (see the conductor client for the pattern).
...
- Launch your app from a login shell, or pass an explicit `PATH`, so version
  ...
  terminal. The conductor does this automatically by asking the user's login
  shell for its `PATH` once and reusing it for spawned backends.
