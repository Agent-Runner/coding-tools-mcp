...
1. **Run through the conductor (`ctc`).** It resolves the user's login-shell
   `PATH` once at startup (the same trick VS Code and kimi-code use) and
   spawns the lower server with it, so nvm-selected toolchains work no matter
   how the host app was launched. Set `CTC_NO_LOGIN_SHELL_PATH=1` to opt out.
